...
classDiagram
    ...
    class RotationReader {
        <<interface>>
        +readRotations(input: String) List~Rotation~
    }

    class StringRotationReader {
        +readRotations(input: String) List~Rotation~
    }

    class RotationScorer {
        <<interface>>
        +score(startPosition: int, rotation: Rotation) long
    }

    class EndAtZeroScorer {
        +score(...) long
    }

    class PassThroughZeroScorer {
        +score(...) long
    }
    ...
    RotationReader <|.. StringRotationReader
    RotationScorer <|.. EndAtZeroScorer
    RotationScorer <|.. PassThroughZeroScorer

    Day01Solver --> RotationReader : inyecta
    Day01Solver --> RotationScorer : inyecta
    Day01Solver ..> Dial : orquesta
```
